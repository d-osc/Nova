// Bun EventEmitter Benchmark
import { EventEmitter } from 'events';

console.log('=== Bun EventEmitter Benchmark ===\n');
...
console.log(`Added ${NUM_LISTENERS} listeners in ${addDuration}ms`);
...
console.log(`Removed ${NUM_LISTENERS} listeners in ${removeDuration}ms`);
...
console.log(`Emitted ${NUM_EMITS} events in ${emitDuration}ms`);
console.log(`Throughput: ${(NUM_EMITS / (emitDuration / 1000)).toFixed(0)} emits/sec`);
console.log(`Total listener calls: ${callCount}\n`);
...
console.log(`Added and triggered ${NUM_ONCE} once-listeners in ${onceDuration}ms`);
console.log(`Throughput: ${(NUM_ONCE / (onceDuration / 1000)).toFixed(0)} ops/sec`);
console.log(`Once calls: ${onceCallCount}\n`);
...
console.log(`Added ${NUM_EVENTS * NUM_LISTENERS_PER_EVENT} listeners across ${NUM_EVENTS} events in ${multiAddDuration}ms`);

// Emit all events
for (let i = 0; i < NUM_EVENTS; i++) {
    emitter4.emit(`event${i}`);
}

const endMulti = Date.now();
const multiEmitDuration = endMulti - midMulti;
console.log(`Emitted ${NUM_EVENTS} different events in ${multiEmitDuration}ms`);
console.log(`Total operations: ${NUM_EVENTS * NUM_LISTENERS_PER_EVENT + NUM_EVENTS}\n`);
...
for (let i = 0; i < 10; i++) {
    emitter5.on('calculate', (a: number, b: number, c: number) => {
        argSum += a + b + c;
    });
}
...
console.log(`Emitted ${NUM_ARG_EMITS} events with 3 arguments in ${argsDuration}ms`);
console.log(`Throughput: ${(NUM_ARG_EMITS / (argsDuration / 1000)).toFixed(0)} emits/sec`);
console.log(`Arg sum: ${argSum}\n`);
...
console.log(`Called listenerCount ${NUM_COUNTS} times in ${countDuration}ms`);
