...
// Number.prototype.toLocaleString() - formats number with locale-specific separators
// Simplified implementation: uses comma as thousands separator (en-US style)
const char* nova_number_toLocaleString(double num) {
    // Handle special values
    if (std::isnan(num)) {
        char* result = new char[4];
        std::strcpy(result, "NaN");
    ...
    char intBuffer[64];
    std::snprintf(intBuffer, sizeof(intBuffer), "%.0f", intPart);
    size_t intLen = std::strlen(intBuffer);

    // Calculate output buffer size (with commas)
    size_t numCommas = (intLen > 0) ? (intLen - 1) / 3 : 0;
    size_t resultSize = intLen + numCommas + 32;  // Extra space for sign, decimal, fraction
    ...
    // Add fractional part if exists (up to 3 decimal places for locale string)
    if (fracPart > 0.0005) {  // Small epsilon to handle floating point
        char fracBuffer[32];
        std::snprintf(fracBuffer, sizeof(fracBuffer), "%.3f", fracPart);
        // Skip the leading "0" from fracBuffer, copy from "."
        char* fracStart = fracBuffer + 1;  // Skip "0", start from "."
        // Remove trailing zeros
        size_t fracLen = std::strlen(fracStart);
        while (fracLen > 2 && fracStart[fracLen - 1] == '0') {
            fracLen--;
        }
