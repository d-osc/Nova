...
function toBeGTE(actual: number, expected: number): number {
    if (actual >= expected) return 0;
    return 1;
}

// expect(actual).toBeLessThanOrEqual(expected)
function toBeLTE(actual: number, expected: number): number {
    if (actual <= expected) return 0;
    return 1;
}
...
function main(): number {
    ...
    fail = fail + toBeGTE(5, 5);
    fail = fail + toBeGTE(6, 5);

    // describe("toBeLessThanOrEqual")
    fail = fail + toBeLTE(5, 5);
    fail = fail + toBeLTE(4, 5);
